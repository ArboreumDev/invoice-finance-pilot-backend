...
def test_admin_update_value_success(whitelist_and_invoices):
    (inv_id1, order_ref1), GURUGRUPA_CUSTOMER_ID, p1, db_session, auth_header = whitelist_and_invoices

    # should add new invoice to db
    invoices = invoice_service.get_all_invoices(db_session)
...
def test_only_admin_can_update(db_session: Session):
    reset_db(deleteWhitelist=True)
