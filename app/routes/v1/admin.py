...
from database.crud.kycuser_service import (ManualVerification,
                                           VerificationStatus)
from database.exceptions import (NoDocumentsException,
...
def _update_document_verification_status(
    phone_number: str, doc_type: ManualVerification, new_status: VerificationStatus, db: Session = Depends(get_db)
):
    try:
        return kycuser_service.update_document_verification_status(phone_number, doc_type, new_status, db)
    except UnknownPhoneNumberException as e:
        raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail=str(e))
...
def _update_verification_status(phone_number: str, newStatus: KYCStatus, db: Session = Depends(get_db)):
    try:
