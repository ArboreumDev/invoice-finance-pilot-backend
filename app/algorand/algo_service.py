...
from utils.common import FinanceStatus, NewAssetCreationInput, FundedInvoice
...
class AlgoService():
    ...
    def get_invoices_to_be_tokenized(self, loan_id, db):
        ...
        invoices_from_loan = [i for i in all_invoices if i.payment_details['loan_id'] == loan_id]
        if not invoices_from_loan:
            raise NoInvoicesToBeTokenized()
        
        # verify no invoice is already tokenized
        invoices_already_tokenized = [i.order_ref for i in all_invoices if "asset_id" in i.payment_details.keys()]
        ...
        invoices_not_in_state_financed = [
            (i.order_ref, i.finance_status) for i in all_invoices if i.finance_status != FinanceStatus.FINANCED
        ]
        ...
        invoices_without_tx_ref = [
            i.order_ref for i in all_invoices if not(bool(i.payment_details.get('disbursal_transaction_id', 0)))
        ]
    ...
    def tokenize_loan(self, loan_id: str, db: Session):
        ...
        compact_invoice_info = [
            FundedInvoice(
                ...
                order_id=i.order_id,
                value=i.value,
                transaction_ref=i.payment_details.get("disbursal_transaction_id"),
        ...
        url = self.base_url + "/log/new"
        # response = requests.request("POST", url, json={"investor_id": investor_ids})
        response = requests.request("POST", url, json={"loan_params": new_asset_input}, headers=self.headers)
        print('resp', response)
...
# TODO load from env
ALGO_LOG_BASE_URL='localhost:8001'
