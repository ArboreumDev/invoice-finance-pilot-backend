...
class Coupon:
    def __init__(
        ...
        decrease for the senior tranche.
        ...
        if APR_corp<=0 or APR_sprt<=0:
          raise NotImplementedError("APRs must be >0 for now")
        ...
        if bool(collection_dates):
            ...
            self.indx_prd_2_collect = [
                int(np.floor((date - collection_dates[0]).days / self.days_per_period)) for date in collection_dates[1:]
            ]
        else:
            self.indx_prd_2_collect = np.arange(loan_tnr % collection_freq, loan_tnr + 1, collection_freq)
    ...
    def _calc_eqn_matrices(self, prv_pmnts_corp=[], prv_pmnts_sprt=[], ideal_pmnts_corp=[], ideal_pmnts_sprt=[]):
        """
        ...
        if k > 0:
            # normalize first period by fst_prd_lemn
            ...
            prv_intr_sprt[0] = prv_intr_sprt[0] * fst_prd_len
            prv_intr_corp = np.array([(1 + PPR_corp) ** (loan_tnr - t) for t in np.arange(1, k + 1)])
            prv_intr_corp[0] = prv_intr_corp[0] * fst_prd_len
        else:
            prv_intr_sprt = []
            prv_intr_corp = []
            penalty_sprt = 0
        ...
        LHS_sprt = prcp_sprt * (1 + PPR_sprt) ** (loan_tnr - 1) * (1 + fst_prd_len * PPR_sprt) - np.dot(
            prv_pmnts_sprt - penalty_sprt, prv_intr_sprt
        )
        LHS_sprt = LHS_sprt * (LHS_sprt > tol)
        LHS_corp = prcp_corp * (1 + PPR_corp) ** (loan_tnr - 1) * (1 + fst_prd_len * PPR_corp) - np.dot(
            prv_pmnts_corp - penalty_corp, prv_intr_corp
        )
        LHS_corp = LHS_corp * (LHS_corp > tol)

        # compute coefficients (if k>0 then first payment already included in LHS)
        coef_intercept_sprt = (
            fst_prd_len * (1 + PPR_sprt) ** (loan_tnr - 1) * (k == 0)
            + ((1 + PPR_sprt) ** (loan_tnr - max(k, 1)) - 1) / PPR_sprt
        )
        coef_intercept_corp = (
            fst_prd_len * (1 + PPR_corp) ** (loan_tnr - 1) * (k == 0)
        ...
        RHS_mtx = np.array([[coef_intercept_sprt, coef_slope_sprt], [-coef_intercept_corp, -coef_slope_corp]])

        return LHS_mtx, RHS_mtx, intercept_coef_mtx, slope_coef_mtx

    def _create_soln_functions(self, prv_pmnts_corp, prv_pmnts_sprt, ideal_pmnts_corp, ideal_pmnts_sprt):
        """
        ...
        # do the math
        LHS_mtx, RHS_mtx, intercept_coef_mtx, slope_coef_mtx = self._calc_eqn_matrices(
            prv_pmnts_corp, prv_pmnts_sprt, ideal_pmnts_corp, ideal_pmnts_sprt
        )
        ...
        def calc_soln_from_slope(slope):
            soln_mtx = np.linalg.solve(intercept_coef_mtx, LHS_mtx - slope * slope_coef_mtx)
        ...
        def calc_soln_from_slack(slack):
            ...
            LHS_mtx = np.array(
                [LHS_sprt + coef_slope_sprt * slope_subsprt, LHS_corp - coef_intercept_corp * intercept_subsprt]
            )
        ...
        def solve_slope_from_intercept(min_intercept_sprt):
            LHS_mtx = np.array([LHS_sprt - min_intercept_sprt * coef_intercept_sprt, LHS_corp])
            RHS_mtx = np.array([[0, coef_slope_sprt], [coef_intercept_corp, -coef_slope_corp]])
            try:
                soln_mtx = np.linalg.solve(RHS_mtx, LHS_mtx)
            except Exception:
    ...
    def _optim_slope(self, k, max_slope, min_slope, calc_soln_from_slope):
        ...
        rslt = optimize.differential_evolution(
            max_intercept_dist,
    ...
    def _optim_slack(self, k, calc_soln_from_slack):
        ...
        rslt = optimize.differential_evolution(
            min_slack, bounds=[(0, self.prcp_corp)], constraints=(nlc3, nlc4), workers=self.num_CPUs
        )
    ...
    def _calc_tranche_pmnts(
        ...
        if k > 0:
            ...
            if (
                np.abs(
                    (ideal_pmnts_corp[k - 1] + ideal_pmnts_sprt[k - 1])
                    - (prv_pmnts_corp[k - 1] + prv_pmnts_sprt[k - 1])
                )
                < tol
            ):
                ...
                if not np.isnan(ideal_pmnts_sprt[k]) and not np.isnan(ideal_pmnts_corp[k]):
                    ...
                    return (
                        np.append(prv_pmnts_sprt, ideal_pmnts_sprt[k:]),
        ...
        (
            calc_soln_from_slope,
            calc_soln_from_slack,
            solve_slope_from_intercept,
            intercept_coef_mtx,
        ) = self._create_soln_functions(prv_pmnts_corp, prv_pmnts_sprt, ideal_pmnts_corp, ideal_pmnts_sprt)
        ...
        ## payments have occurred
            ...
            if (ideal_pmnts_corp[k - 1] + ideal_pmnts_sprt[k - 1]) > (prv_pmnts_corp[k - 1] + prv_pmnts_sprt[k - 1]):
                min_intercept_sprt = ideal_pmnts_sprt[max(0, k - 2)]
                max_slope, max_intercept_corp = solve_slope_from_intercept(min_intercept_sprt)
            ...
            ## payments more than ideal
                ...
                min_intercept_sprt = ideal_pmnts_sprt[max(0, k - 1)]
                max_slope, max_intercept_corp = solve_slope_from_intercept(min_intercept_sprt)
    ...
    def _calc_prcp_intr_pnlt(self, pmnt_vec, PPR, principal, ideal_pmnt_vec):
        ...
        # Inflate first payment by the first period length
        # (amount that would have been paid if the standard period length)
        pmnt_vec[0] = pmnt_vec[0] / self.fst_prd_len
        ideal_pmnt_vec[0] = ideal_pmnt_vec[0] / self.fst_prd_len

        # calculate interest and principal per period
        for i in range(0, self.loan_tnr):
            # interest per period
            intr_perPrd[i] = np.maximum(0, PPR * (principal - np.sum(prcp_perPrd[0:i])))
            # penalty per period
            ...
            # multiply first terms by fst_prd_len
            if i == 0:
                intr_perPrd[i] = intr_perPrd[i] * self.fst_prd_len
                pnlt_perPrd[i] = pnlt_perPrd[i] * self.fst_prd_len
                prcp_perPrd[i] = prcp_perPrd[i] * self.fst_prd_len
    ...
    def _checksum_payments(self, pmnt_vec, orig_prv_pmnts, prcp_perPrd, intr_perPrd, pnlt_perPrd, principal):
        ...
        if prcp_perPrd.sum() != principal:
            coef = (principal - prcp_perPrd[0 : self.num_prv_pmnts].sum()) / prcp_perPrd[self.num_prv_pmnts :].sum()
        ...
        if (self.loan_tnr - self.num_prv_pmnts) > 1 or (
            intr_perPrd[self.num_prv_pmnts] + prcp_perPrd[self.num_prv_pmnts]
        ) > pmnt_vec[self.num_prv_pmnts]:
            intr_perPrd = pmnt_vec - prcp_perPrd - np.abs(pnlt_perPrd)

        return pmnt_vec, prcp_perPrd, intr_perPrd, pnlt_perPrd

    def _collections_2_periods(self, ideal_pmnts_sprt, ideal_pmnts_corp, start_collect=0, end_collect=None):
        """
        ...
        if self.collection_freq == 1:
            ...
            for i in range(start_collect, min(end_collect, self.num_prv_collect)):
                ...
                self.prv_pmnts_sprt[pmnt_indx_str:pmnt_indx_end] = calc_pmnts_per_collect(
                    self.prv_collect_sprt[i], ideal_pmnts_sprt[pmnt_indx_str:pmnt_indx_end]
                )
                # convert for corpus
                self.prv_pmnts_corp[pmnt_indx_str:pmnt_indx_end] = calc_pmnts_per_collect(
                    self.prv_collect_corp[i], ideal_pmnts_corp[pmnt_indx_str:pmnt_indx_end]
                )
    ...
    def calc_tranche_coupon(self, start_period: int = 0):
        ...
        for i in range(start_period, min(self.num_prv_pmnts + 1, self.loan_tnr)):
            pmnt_sprt, pmnt_corp, slope, intercept_sprt, intercept_corp, slack = self._calc_tranche_pmnts(
                self.prv_pmnts_corp[0:i],
        ...
        corp_prcp_perPrd, corp_intr_perPrd, corp_pnlt_perPrd = self._calc_prcp_intr_pnlt(
            pmnt_corp, self.PPR_corp, self.prcp_corp, ideal_pmnts_corp
        )

        sprt_prcp_perPrd, sprt_intr_perPrd, sprt_pnlt_perPrd = self._calc_prcp_intr_pnlt(
            pmnt_sprt, self.PPR_sprt, self.prcp_sprt, ideal_pmnts_sprt
        )

        # adjust vectors to ensure principal exactly sums to actual principal
        pmnt_corp, corp_prcp_perPrd, corp_intr_perPrd, corp_pnlt_perPrd = self._checksum_payments(
            pmnt_corp, self.prv_pmnts_corp, corp_prcp_perPrd, corp_intr_perPrd, corp_pnlt_perPrd, self.prcp_corp
        )

        pmnt_sprt, sprt_prcp_perPrd, sprt_intr_perPrd, sprt_pnlt_perPrd = self._checksum_payments(
            pmnt_sprt, self.prv_pmnts_sprt, sprt_prcp_perPrd, sprt_intr_perPrd, sprt_pnlt_perPrd, self.prcp_sprt
        )
    ...
    def _periods_2_collections(self, vec=None):
        ...
        def splitsum(vec):
            if self.collection_freq == 1:
                ...
                return np.fromiter(map(np.sum, np.split(vec, self.indx_prd_2_collect[1:-1])), dtype=np.float)
        ...
        if vec is not None:
            ...
            self.corp_collect_full_repay = self.corp_intr_perCollect[self.num_prv_collect] + self.corp_prcp_owed
            self.sprt_collect_full_repay = self.sprt_intr_perCollect[self.num_prv_collect] + self.sprt_prcp_owed
    ...
    def _coupon_zero(self):
        ...
        self.corp_pmnts = (
            ...
            self.sprt_pmnts
        ) = self.ideal_pmnts_sprt = self.sprt_prcp_perPrd = self.sprt_intr_perPrd = self.sprt_pnlt_perPrd = np.zeros(
            self.loan_tnr
        )

        self.sprt_pmnt_current = (
            self.corp_pmnt_current
        ) = self.intercept_sprt = self.intercept_corp = self.slope = self.slack = 0
    ...
    def _convert_repayments_representation(self, *args):
        ...
        if len(args) == 1 and isinstance(args[0], dict):
            ...
            collections_corp = [
                (self.start_date + dt.timedelta(days=key * self.days_per_period), val[0])
            ...
            if all(isinstance(item, tuple) for item in (collections_corp + collections_sprt)):
                ...
                collections_corp = {
                    self.indx_prd_2_collect[i + 1]: collections_corp[i] for i in range(0, len(collections_corp))
                }
                collections_sprt = {
                    self.indx_prd_2_collect[i + 1]: collections_sprt[i] for i in range(0, len(collections_sprt))
                }

            # coalesce corpus and supporter dicts into one
            repayments_dict = {prd: tuple((collections_corp[prd], collections_sprt[prd])) for prd in collections_corp}

            return repayments_dict

    def extend_loan_tnr(self, collect_prds_2_extend: int, new_pnlt_APR: float = None):
        """
        ...
        # map compounding periods to collection periods
        new_collection_prds = (
            np.arange(self.collection_freq, self.collection_freq * collect_prds_2_extend + 1, self.collection_freq)
            + self.loan_tnr_orig
        )
        ...
        # vectors storing ideal past/future payments
        self.ideal_pmnts_corp = np.pad(
            self.ideal_pmnts_corp,
            (0, self.loan_tnr - len(self.ideal_pmnts_corp)),
            mode="constant",
            constant_values=np.nan,
        )
        self.ideal_pmnts_sprt = np.pad(
    ...
    def update(self, collections_corp: List = [], collections_sprt: List = []):
        """
        ...
        original = set(self.collection_dict.items())
        combined = set({**self.collection_dict, **repayments_dict}.items())
        diff_items = combined.difference(original)

        # return which periods are different
        prd_diff = [prd for prd, item in enumerate(sorted(combined, key=lambda x: x[0])) if item in diff_items]
        ...
        if bool(prd_diff):
            ...
            # update
            self.prv_collect_corp = np.array(
                [val[0] for key, val in self.collection_dict.items() if key <= prd_end and key > 0]
            )
            self.prv_collect_sprt = np.array(
                [val[1] for key, val in self.collection_dict.items() if key <= prd_end and key > 0]
            )
            ...
            # append to payments
            self.prv_pmnts_corp = np.pad(
                self.prv_pmnts_corp,
                (0, self.num_prv_pmnts - len(self.prv_pmnts_corp)),
                mode="constant",
                constant_values=np.nan,
            )
            self.prv_pmnts_sprt = np.pad(
                self.prv_pmnts_sprt,
            ...
            if np.isnan(self.prv_pmnts_corp[-1]):
                ...
                start_period = self.indx_prd_2_collect[self.num_prv_pmnts]
